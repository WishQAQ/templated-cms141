...
class ListView
{
    ...
    function GetPageListST($list_len,$listitem="index,end,pre,next,pageno")
    {
        ...
        {
            return "<span class=\"pages\">共 ".$this->TotalResult."页，".$this->TotalResult."条记录</span>\r\n";
        }
        if($this->TotalResult == 0)
        {
            return "<span class=\"pages\">共 0 页，".$this->TotalResult."条记录</span>\r\n";
        }
        $purl = $this->GetCurUrl();
        $maininfo = "<span class=\"pages\">共 {$totalpage} 页，".$this->TotalResult."条</span>\r\n";
        ...
        if($this->PageNo != 1)
        {			
            $prepage.="<a class=\"page larger\" href='".str_replace("{page}",$prepagenum,$tnamerule)."'>上一页</a>";
            $indexpage="<a class=\"page larger\" href='".str_replace("{page}",1,$tnamerule)."'>首页</a>";			
        }
        else
        {
            $indexpage="";
        ...
        {
			$nextpage.="<a class=\"page larger\" href='".str_replace("{page}",$nextpagenum,$tnamerule)."'>下一页</a>";
			$endpage="<a class=\"page larger\" href='".str_replace("{page}",$totalpage,$tnamerule)."'>末页</a>";
        }
        else
        {
            $endpage="";
        ...
        for($j;$j<=$total_list;$j++)
        {
            ...
            {
                $listdd.="<a class=\"page larger\" href='".str_replace("{page}",$j,$tnamerule)."'>".$j."</a>";				
            }
        }
        $plist = '';
    ...
    function GetPageListDM($list_len,$listitem="index,end,pre,next,pageno")
    {
        ...
        if($totalpage<=1 && $this->TotalResult>0)
        {			
            return "<span class=\"pages\">共 1 页/".$this->TotalResult." 条记录</span>\r\n";
        }
        if($this->TotalResult == 0)
        {
            return "<span class=\"pages\">共 0 页/".$this->TotalResult." 条记录</span>\r\n";
        }
        $maininfo = "<span class=\"pages\">共 {$totalpage} 页<strong>".$this->TotalResult."条</span>\r\n";
        ...
        if($this->PageNo != 1)
        {
			$prepage.="<a class=\"page larger\" href='".$purl."PageNo=$prepagenum'>上一页</a>";
			$indexpage="<a class=\"page larger\" href='".$purl."PageNo=1'>首页</a>";
        }
        else
        {
            $indexpage="";
        ...
        if($this->PageNo!=$totalpage && $totalpage>1)
        {
			$nextpage.="<a class=\"page larger\" href='".$purl."PageNo=$nextpagenum'>下一页</a>";
            $endpage="<a class=\"page larger\" href='".$purl."PageNo=$totalpage'>末页</a>";
        }
        else
        {
            $endpage="";
        ...
        for($j;$j<=$total_list;$j++)
        {
            ...
            {
				$listdd.="<a class=\"page larger\" href='".$purl."PageNo=$j'>".$j."</a>";
            }
        }
